// Patches the Electron.app bundle in node_modules to show "Jarvis" name and icon in macOS dock during dev mode.
...
if (existsSync(plistPath)) {
  try {
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleName Jarvis" "${plistPath}"`)
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleDisplayName Jarvis" "${plistPath}"`)
    console.log("[patch-electron-dev] Updated Info.plist: name -> Jarvis")
  } catch (e) {
