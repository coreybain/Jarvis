...
export type WorktreeConfigSource = "custom" | "cursor" | "jarvis" | null
...
const ONECODE_CONFIG_PATH = ".jarvis/worktree.json"
 ...
 * Priority: custom path > .cursor/worktrees.json > .jarvis/worktree.json
 */
export async function detectWorktreeConfig(
  ...
  if (await fileExists(onecodePath)) {
    ...
    if (config) {
      return { config, path: onecodePath, source: "jarvis" }
    }
  }
...
export async function saveWorktreeConfig(
  ...
  target: "cursor" | "jarvis" | string = "jarvis",
