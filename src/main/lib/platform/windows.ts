...
export class WindowsPlatformProvider extends BasePlatformProvider {
  ...
  getCliConfig(): CliConfig {
    ...
    return {
      installPath: path.join(home, ".local", "bin", "jarvis.cmd"),
      scriptName: "jarvis.cmd",
  ...
  async installCli(
    ...
    try {
      ...
      console.log("[CLI] Installed jarvis command to", installPath)
      console.log(
      ...
      return {
        success: true,
        pathHint: `To use jarvis from terminal, add to your PATH: ${installDir}`,
      }
    } catch (error: unknown) {
      const errorMessage =
  ...
  async uninstallCli(): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      ...
      console.log("[CLI] Uninstalled jarvis command")
      return { success: true }
    } catch (error: unknown) {
      const errorMessage =
