  ...
  return (
    ...
    >
      ...
      <TrafficLights
        ...
        isDesktop={isDesktop}
        className="absolute -left-[24px] top-[12px] z-20"
      />
...
export function AgentsSidebar({
  ...
  const handleOpenThreadsView = useCallback(() => {
    ...
    const previousChat = agentChats?.find(
      (chat) =>
        chat.id === previousChatId || chat.id === `remote_${previousChatId}`,
    );
  ...
  const sidebarContent = (
    ...
    >
      {!isMobileFullscreen && (
        <div
          className={cn(
            "w-11 flex-shrink-0 border-r border-border/50 bg-background/40 px-1.5 pb-2 flex flex-col pt-1",
          )}
          data-sidebar-content
        >
          <TrafficLightSpacer
              ...
              <Tooltip delayDuration={0}>
                <TooltipTrigger asChild>
                  <button
                    ...
                    onClick={handleOpenAutomationsView}
                    className={cn(
                      "flex items-center justify-center h-8 w-8 rounded-md transition-[background-color,color,transform] duration-150 ease-out active:scale-[0.97] outline-offset-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-ring/70",
                      isAutomationsView
                        ? "bg-foreground/10 text-foreground"
